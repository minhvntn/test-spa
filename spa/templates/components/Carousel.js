...
function renderCarousel({ content }) {
  ...
  useEffect(() => {
    const interval = magnoliaContext.isMagnoliaEdit ? null : setInterval(next, 5000);
      ...
      {length > 0 && (
        <>
          {content['@nodes'].map((nodeName, i) => {
            ...
            if (i === active) className += ' Carousel__Item--active';
...
function Carousel(props) {
  ...
  return items ? <EditableArea content={items} customView={renderCarousel} /> : null;
}
